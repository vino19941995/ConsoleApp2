...
pipeline {
			...
			stages {
				stage('Source'){
					steps{
						checkout([$class: 'GitSCM', branches: [[name: '*/master']], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[credentialsId: '1234567890', url: 'http://http://192.168.0.100/vino/aspdotnetjenkins']]])
					}
				}
				stage('Build') {
    					steps {
